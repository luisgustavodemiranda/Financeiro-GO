...
Implementação integrada pelo [PR #5](https://github.com/luisgustavodemiranda/Financeiro-GO/pull/5). Validação posterior registrada em `feature/fase-4-validacao-faturas`, criada de develop após esse merge. Migration 0003 aplicada com autorização nos dois bancos e integração real concluída.
...
Inspeção somente leitura em 25/09/2026 confirmou nos dois bancos exclusivos: histórico 0001/0002 com hashes esperados, usuários próprios sem superusuário, permissão CREATE no schema, estrutura de cards esperada e ausência de invoices/purchases. A inspeção foi repetida imediatamente antes da aplicação autorizada, confirmando os destinos e histórico esperado.

Após autorização específica em 25/09/2026, o executor aplicou somente 0003 em `financeiro_go` e `financeiro_go_test`, preservando 0001/0002. A inspeção posterior confirmou invoices/purchases e o histórico com checksum de 0003 `4940ae487a9917da408a38eccb9cfeab38f3b13b082128541f0a4519969cdd26`. A integração executou apenas no segundo banco, limpando somente as próprias fixtures. O executor valida o histórico e usa transação. Esta versão da API recusa iniciar em PostgreSQL enquanto 0003 estiver pendente. O binário anterior também rejeita histórico com versão desconhecida; planejar parada da API para migration e atualização do binário. Não há downgrade automático.
...
A suíte de comportamento é compartilhada entre memória e o teste opt-in PostgreSQL. A integração real passou com FINANCEIRO_TEST_DATABASE_URL configurada para financeiro_go_test: suíte compartilhada de faturas, concorrência, erros sem gravação, HTTP e persistência por outro pool. A suíte completa go test ./... -count=1 também aprovou a regressão de contas e cartões; vet e build passaram. A API iniciou com PostgreSQL no banco da aplicação após verificar 0003, sem inserir fixtures nele. O CI não acessa esses bancos locais; sua evidência é distinta. Sem a variável de integração, testes de banco continuam SKIP. Nenhuma pendência de banco permanece neste incremento. Próximo incremento recomendado após essa conclusão: definir e implementar pagamento integral com operação atômica entre conta e fatura, sem nova despesa.
